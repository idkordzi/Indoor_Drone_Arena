if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--detect", type=int, choices=[0, 1, 2, 3], default=0,
                        help="Enables datection: 0 - disabled; 1 - send frames only; "
    ...
    if args.calibrate == 0 and args.detect == 0:
        args.detect = 1
    if args.calibrate != 0 and args.detect != 0:
        print("MAIN::WARNING: Both options 'calibrate' and 'detect' were enabled - only 'detect' "
              "will be choosen")
    if args.listen == 1:
        print("MAIN::INFO: 'Listener' mode has been enabled, all other modes will be disabled")
    
    from mocap.device import MocapCamera
    edge = MocapCamera("configs/config.yaml", args.network == 1)
