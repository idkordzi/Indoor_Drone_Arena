...
def new_process_calculate_projection_matrix(
    ...
    while True:
        ...
        for i in [0,1,2]: blank[:,:,i] = frame
        for x, y, r in objs:
            cv2.circle(blank, (int(x), int(y)), int(r*3), (0,0,255), 4)
        if flag_en:
            for idx, x, y in sorted_markers:
                cv2.putText(blank, f"{idx+1}", (int(x)+10, int(y-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 4)
        blank = cv2.resize(blank, None, fx=scale_display, fy=scale_display)
        cv2.putText(blank, f"DET STRUCT SHAPE: {np.array(objs).shape}", (30,10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2)
        cv2.imshow("frame", blank)
        resp = cv2.waitKey(1)
        if resp == ord('s') and flag_en:
